...
// function getStyle(element, att){
    ...
    if(window.getComputedStyle){
        ...
        return window.getComputedStyle(this.dom)[att];

    }else{

        //针对IE9以下兼容

        return this.dom.currentStyle[att];
...
/*        var style=null;
    ...
    // 多个属性运动框架  添加回调函数
/*    animate:function (json,fn) {  // 给谁    json
        ...
        this.dom.timer = setInterval(function() {
            ...
            for(var attr in json){   // attr  属性     json[attr]  值
                ...
                if(attr == "opacity")
                {
                    current = Math.round(parseInt(jq().getCss(attr)*100)) || 0;
                }
                else
                {
                	console.log(jq());
                   current=parseInt(jq().getCss(attr));
                }
               // console.log(current);
                 // 目标位置就是  属性值
                ...
                {
                     if("opacity" in this.dom.style)  // 判断 我们浏览器是否支持opacity
                     {
                         // obj.style.opacity
                         this.dom.style.opacity = (current + step) /100;
                     }
                    else
                     {  // obj.style.filter = alpha(opacity = 30)
                         this.dom.style.filter = "alpha(opacity = "+(current + step)* 10+")";

                     }
                }
                else if(attr == "zIndex")
                {
                    this.dom.style.zIndex = json[attr];
                }
                else
                {
                    this.dom.style[attr] = current  + step + "px" ;
                }
